...
class TruthtableDB():
    ...
    def get_desired_seq(self):
        ...
        for seq_r in seqrn.seqrow:
            print(f'Extracing sequence {seqrn.seqnum[seqrn.seqrow.index(seq_r)]} from truthtable {self._tt_name}...')
            seq = [1]
            next_sn = -1
            tick = 1
            try:
                ...
                seq_num_lst += [seqrn.seqnum[seqrn.seqrow.index(seq_r)]] *len(seq)
                seq_lst.extend(seq)
    ...
    def parse_truthtable(self):
        parsed_tt = pd.DataFrame(list(zip(self._tt_name_lst, self._seq_num_lst, self._seq, self._step_name,
                                        self._eos_english, self._next_steps,
                                        self._true_devices)), 
                                        columns=['Truthtable', 'Sequence Number','Step Number', 'Step Name', 

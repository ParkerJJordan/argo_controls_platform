...
def test_parse_discrete_io():
    io = parse_discrete_io(word_num=145, bit_num=1, state_num=0)
    assert io.address == 'rack 14 mod 5 bit 1 (Real_Inputs[145].1)'
    assert io.state == False

    io = parse_discrete_io(word_num=135, bit_num=3, state_num=1)
    assert io.address == 'rack 13 mod 5 bit 3 (Real_Inputs[135].3)'
    assert io.state == True

    io = parse_discrete_io(word_num=10, bit_num=3, state_num=2)
    assert io.address == 'Binary_EOS[10].3'
    assert io.state == False

    io = parse_discrete_io(word_num=110, bit_num=3, state_num=3)
...
def test_parse_analog():
    assert parse_analog(
        analog_num=678, greater_than=1, analog_preset=34
    ) == 'Analog_Variables[678] is greater than or equal to 34'

    assert parse_analog(
        analog_num=92, greater_than=0, analog_preset=1000
    ) == 'Analog_Variables[92] is less than or equal to 1000'


def test_parse_time():
    assert parse_time(
        time_preset=135
    ) == 'the time in step exceeds 2.25 minutes (135 seconds)'
    ...
    assert parse_time(
        time_preset=3600
    ) == 'the time in step exceeds 1.00 hours (60.00 minutes)'
...
def test_parse_process_step_range():
    ...
    assert parse_process_step_range(
        process_num=process, low_step=step1, high_step=step2,
        in_range=True) == 'any unit of process 30 is between steps 21 and 29'
    assert parse_process_step_range(
        process_num=process, low_step=step1, high_step=step2, in_range=False
    ) == 'any unit of process 30 is not between steps 21 and 29'
...
def test_parse_process_either_step():
    ...
    step1 = 21
    step2 = 29
    assert parse_process_either_step(
        process_num=process, step_num1=step1, step_num2=step2,
    ...
    step1 = 29
    assert parse_process_either_step(
        process_num=process, step_num1=step1,
...
def test_parse_process_both_steps():
    ...
    assert parse_process_both_steps(
        process_num=process, step_num1=step1, step_num2=step2,
        in_step=True) == 'any two units of process 30 are in steps 21 and 29'
    assert parse_process_both_steps(
        process_num=process, step_num1=step1, step_num2=step2, in_step=False
    ) == 'any two units of process 30 are not in steps 21 and 29'
...
def test_parse_unit_step_range():
    ...
    assert parse_unit_step_range(
        unit_num=unit, low_step=step1, high_step=step2,
...
def test_parse_unit_either_step():
    ...
    step1 = 21
    step2 = 29
    assert parse_unit_either_step(
        unit_num=unit, step_num1=step1, step_num2=step2,
    ...
    unit = 30
    step1 = 29
    assert parse_unit_either_step(unit_num=unit, step_num1=step1,
...
def test_parse_unit_seq_range():
    ...
    assert parse_unit_seq_range(
        unit_num=unit, low_seq=seq1, high_seq=seq2,
...
def test_eos_4():
    assert eos_4(eos_mod_7=135
                 ) == 'the time in step exceeds 2.25 minutes (135 seconds).'


def test_eos_5():
    assert eos_5(eos_mod_1=12, eos_mod_2=35,
                 eos_mod_3=50) == 'unit 12 is between steps 35 and 50.'


def test_eos_6():
    assert eos_6(eos_mod_1=14, eos_mod_2=36,
...
def test_eos_23():
    assert eos_23(
        eos_mod_1=30, eos_mod_2=21,
        eos_mod_3=29) == 'any two units of process 30 are in steps 21 and 29.'
...
def test_eos_30():
    assert eos_30(
        eos_mod_1=174, eos_mod_2=0, eos_mod_7=135, eos_mod_8=1110
...
def test_eos_37():
    assert eos_37(
        ...
        eos_mod_6=2,
        eos_mod_7=3660
    ) == 'rack 26 mod 6 bit 14 (Real_Inputs[266].14) is False or the time in step exceeds 1.02 hours (61.00 minutes), and Binary_EOS[322].13 is False.'
...
def test_eos_39():
    assert eos_39(
        eos_mod_1=174, eos_mod_2=7, eos_mod_3=0, eos_mod_7=135, eos_mod_8=4000
...
def test_eos_43():
    assert eos_43(
        eos_mod_1=10, eos_mod_2=21, eos_mod_3=23, eos_mod_4=1, eos_mod_7=135
...
def test_eos_45():
    assert eos_45(
        eos_mod_1=174, eos_mod_2=7, eos_mod_3=0, eos_mod_4=12, eos_mod_7=135
...
def test_eos_48():
    assert eos_48(
        ...
        eos_mod_6=3,
        eos_mod_7=20
    ) == 'current unit is between sequence 2 and 6 and rack 12 mod 3 bit 13 (Real_Inputs[123].13) is True and the time in step exceeds 0.33 minutes (20 seconds) then transfer to step 3, or if the time in step exceeds 0.33 minutes (20 seconds) then the GCC continues.'


def test_eos_73():
    assert eos_73(
        eos_mod_1=342, eos_mod_2=5, eos_mod_3=0, eos_mod_7=135
...
def test_eos_77():
    assert eos_77(
        eos_mod_1=34, eos_mod_2=10,
        eos_mod_3=20) == 'any unit of process 34 is between steps 10 and 20.'
...
def test_eos_80():
    assert eos_80(
        ...
        eos_mod_5=13,
        eos_mod_6=2
    ) == 'rack 26 mod 6 bit 14 (Real_Inputs[266].14) is False and Binary_EOS[322].13 is False.'
...
def test_eos_83():
    assert eos_83(
        ...
        eos_mod_6=2,
        eos_mod_7=3660
    ) == 'rack 26 mod 6 bit 14 (Real_Inputs[266].14) is False or Binary_EOS[322].13 is False or the time in step exceeds 1.02 hours (61.00 minutes).'
...
def test_eos_85():
    assert eos_85(
        ...
        eos_mod_7=37,
        eos_mod_8=56
    ) == 'rack 26 mod 6 bit 14 (Real_Inputs[266].14) is False then transfer to step 37 or if Binary_EOS[322].13 is False then transfer to step 56.'
